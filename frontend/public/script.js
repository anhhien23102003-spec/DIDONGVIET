...
const state = {
  ...
  adminLoggedIn: JSON.parse(localStorage.getItem('ddv_admin_logged_in') || 'false'),
  currentCategory: 'all',
  currentBrand: 'all',
  minPrice: 0,
  maxPrice: 999999999,
...
function openAuthModal(mode = 'customer', tab = 'login') {
...
function switchAuthMode(mode) {
  ...
  const customerBtn = document.getElementById('authModeCustomerBtn');
  const adminBtn = document.getElementById('authModeAdminBtn');
  const kicker = document.getElementById('authKicker');
  const title = document.getElementById('authTitle');

  const isCustomer = mode === 'customer';
  customerPanel.style.display = isCustomer ? 'block' : 'none';
  adminForm.style.display = isCustomer ? 'none' : 'block';

  customerBtn.classList.toggle('active', isCustomer);
  adminBtn.classList.toggle('active', !isCustomer);

  kicker.textContent = isCustomer ? 'Khách hàng' : 'Quản trị hệ thống';
  title.textContent = isCustomer ? 'Chào mừng bạn quay lại' : 'Đăng nhập quản trị';
}
...
function ensureAdminAccess(actionName = 'thao tác quản trị') {
  if (!state.adminLoggedIn) {
    openAuthModal('admin');
    showToast(`Chỉ tài khoản admin mới có quyền ${actionName}.`, 'error');
    return false;
  }
  return true;
}
...
function renderRoleLayout() {
  ...
  const adminLabel = document.getElementById('adminSwitchLabel');
  const adminSwitch = document.querySelector('.btn-admin-switch');

  if (!customerView || !adminView) return;

  const isAdminSession = Boolean(state.adminLoggedIn);
  const isCustomerSession = Boolean(state.currentUser);

  customerView.style.display = isAdminSession ? 'none' : 'block';
  adminView.style.display = isAdminSession ? 'block' : 'none';

  if (adminLabel) {
    adminLabel.innerText = isAdminSession ? 'Về Shop' : 'Quản Trị Admin';
  }

  if (adminSwitch) {
    adminSwitch.style.display = isCustomerSession ? 'none' : 'flex';
  }
...
function handleCustomerLogin(e) {
  ...
  localStorage.setItem('ddv_admin_logged_in', JSON.stringify(false));
  updateAuthUI();
  renderRoleLayout();
...
function handleAdminLogin(e) {
  ...
  if (email === 'admin@didongviet.vn' && password === 'admin123') {
    state.adminLoggedIn = true;
    state.currentUser = null;
    localStorage.setItem('ddv_admin_logged_in', JSON.stringify(true));
    localStorage.setItem('ddv_user', JSON.stringify(null));
    closeModal('authModal');
    updateAuthUI();
    renderRoleLayout();
  ...
  showToast('Email hoặc mật khẩu admin không đúng!', 'error');
}
...
function updateAuthUI() {
  ...
  if (adminLabel) {
    adminLabel.innerText = state.adminLoggedIn ? 'Về Shop' : 'Quản Trị Admin';
  }

  if (adminSwitch) {
    adminSwitch.style.display = state.currentUser && !state.adminLoggedIn ? 'none' : 'flex';
  }
}
...
function toggleAdminView() {
  ...
  if (isAdmin) {
    ...
    updateAuthUI();
    renderRoleLayout();
    showToast('Bạn đã quay về trang khách hàng.');
  } else {
...
function openAddProductModal() {
  if (!ensureAdminAccess('thêm sản phẩm')) return;
...
function openEditProductModal(id) {
  if (!ensureAdminAccess('sửa sản phẩm')) return;
...
async function handleSaveAdminProduct(e) {
  e.preventDefault();
  if (!ensureAdminAccess('lưu thay đổi sản phẩm')) return;
...
async function deleteProduct(id) {
  if (!ensureAdminAccess('xóa sản phẩm')) return;
